...
class IntStack4 {
	...
	public int pop() throws EmptyIntStackException {
		if (isEmpty()) // 스택이 빔
			throw new EmptyIntStackException(); // 예외의 종류가 달라 클래스를 분리해놈
		return stk[--ptr];
	}

	//--- 스택에서 데이터를 피크(peek, 정상에 있는 데이터를 들여다봄) ---//
	public int peek() throws EmptyIntStackException {
		//if (ptr<=0) //대신에 사용 : 처음에는 이게 편하지만 밑에 엠티처럼 함수를 불러서하는게 맞다. 작은함수를 구현해서 해라
		if (isEmpty()) // 스택이 빔
	...
	public class 실습4_2정수스택_배열 {
		...
		public static void main(String[] args) {
			...
			IntStack4 s = new IntStack4(10); // 최대 4 개를 푸시할 수 있는 스택
